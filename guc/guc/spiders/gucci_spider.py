...
class GucciSpider(scrapy.Spider):
        ...
        def parse(self,response):
            for kapada  in response.xpath("//div[contains(@class,'product-tiles-grid')]"):
                if kapada.xpath("./article[contains(@class,'product-tiles-grid-item product-tiles-grid-item-medium product-tiles-grid-item-small hover-link border-right')]").extract():
                    count=0
                    count=count+1
                    cloth= Item()
                    ...
                    res=scrapy.Request(url=cloth['link'],callback=self.parse,method=GET)
                    res.meta['item'] = cloth
                    yield cloth
